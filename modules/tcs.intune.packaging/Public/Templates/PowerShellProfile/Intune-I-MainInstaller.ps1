...
$StorageFolderBasePath = Join-Path -Path $APFFullBase -ChildPath $StorageFolderBase
$StorageFolderName = "PowerShellProfiles"
$StorageFolderNamePath = Join-Path -Path $StorageFolderBasePath -ChildPath $StorageFolderName
$StorageFolderProfilePath = $($InstallConfig.directory)
...
if ($Uninstall) {
        ...
        try {
            ...
            if ([string]::IsNullOrEmpty($InstallConfig.files) -eq $false) {
                ...
                foreach ($a in $($InstallConfig.files -Split ',')) {
                    ...
                    try {
                        $Item = Get-ChildItem -Path $(Join-Path -Path $StorageFolderFullPath -ChildPath $a)
    ...
    if ((Test-Path -Path $StorageFolderBasePath) -eq $false) {
        Write-DeploymentLog -Message "Couldn't find storage folder: $($StorageFolderBase), creating now." -MessageType "Info" -LogPath $LoggingPath
        try {
            New-Item -Path $StorageFolderBasePath -ItemType Directory -ErrorAction Stop
        }
        catch {
            Write-DeploymentLog -Message "Failed to create storage folder $($StorageFolderBase), breaking.." -MessageType "Error" -LogPath $LoggingPath
            exit 1
        }
    }
    if ((Test-Path -Path $StorageFolderNamePath) -eq $false) {
        ...
        try {
            New-Item -Path $StorageFolderNamePath -ItemType Directory -ErrorAction Stop
        }
        catch {
            Write-DeploymentLog -Message "Failed to create storage folder $($StorageFolderBase), breaking.." -MessageType "Error" -LogPath $LoggingPath
            exit 1
    ...
    if (Test-Path -Path $StorageFolderFullPath) {
        ...
        if ((Test-Path -Path $StorageFolderFullPath) -eq $false) {
            Write-DeploymentLog -Message "Couldn't find storage folder: $($StorageFolderProfilePath), creating now." -MessageType "Info" -LogPath $LoggingPath
            try {
