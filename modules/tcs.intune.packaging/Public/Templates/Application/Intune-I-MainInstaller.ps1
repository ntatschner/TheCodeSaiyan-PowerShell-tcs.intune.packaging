...
if ($Uninstall) {
    if ($InstallConfig.uninstallpath -eq "") {
        ...
        if ($InstallConfig.target -eq "user") {
            ...
            if (-Not ($InstallConfig.uninstallpath -match '^[a-zA-Z]:\\')) {
                $Script:UninstallPath = Join-Path -Path $PSScriptRoot -ChildPath $InstallConfig.uninstallpath
            } else {
    ...
    if ($UninstallPath -match ".msi") {
        try {
            $UninstallSplat = @{
                ...
                WorkingDirectory = $(if (Test-Path -Path $UninstallPath -PathType Leaf) { Split-Path -Path $UninstallPath -Parent } else { $UninstallPath })
            ...
            $Script:Process = Start-Process @UninstallSplat
        ...
        try {
            $UninstallSplat = @{
                FilePath         = "cmd.exe"
                WorkingDirectory = $(if (Test-Path -Path $UninstallPath -PathType Leaf) { Split-Path -Path $UninstallPath -Parent } else { $UninstallPath })
                ArgumentList     = "/c `"$UninstallPath`" $($InstallConfig.uninstallswitches)"
                NoNewWindow      = $true
                Wait             = $true
                ErrorAction      = "Stop"
                PassThru         = $true
